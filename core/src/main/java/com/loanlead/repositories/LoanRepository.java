...
public interface LoanRepository extends PagingAndSortingRepository<Loan, Integer> {
    ...
    @Query("SELECT l FROM Loan l WHERE l.stage < 12 AND l.createdAt BETWEEN :minDate AND :maxDate")
    ...
    @Query("SELECT a FROM Auditing a, Loan l WHERE l.id = a.loan AND l.createdAt BETWEEN :minDate AND :maxDate ORDER BY a.loan, a.createdAt")
    ...
    @Query("SELECT l FROM Loan l, User u WHERE l.user = u.id AND u.employeeId = users_branches.employee_id AND users_branches.branch_name = branches.name AND branches.id = :entityId AND l.createdAt BETWEEN :minDate AND :maxDate")
    ...
    @Query("SELECT DISTINCT l FROM Loan l, User u WHERE l.role IN :roles AND l.user = users_branches.employee_id AND users_branches.branch_name IN :branches AND l.status IN ('Forwarded', 'Deferred')")
    ...
    @Query("SELECT l FROM Loan l, User u WHERE l.role IN :roles AND l.user = users_branches.employee_id AND users_branches.branch_name AND l.status NOT IN ('Forwarded', 'Rejected', 'Deferred', 'Disbursed') ORDER BY l.receiveTimestamp DESC")
    ...
    @Query("SELECT max(l.id) FROM Loan l WHERE l.stage = 12")
    public Integer findMaxIdForDisbursedLoans();

    @Query("SELECT min(l.id) FROM Loan l WHERE l.stage = 12")
